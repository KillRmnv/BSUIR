...
\begin{table}[H]
	...
	\begin{tabular}{|p{3.5cm}|p{3cm}|p{6cm}|}
		...
		\hline
		\texttt{/api/summarize-doc} & POST & Реферирование документа по ID: принимает \texttt{doc\_id}, \texttt{method}, \texttt{n\_sentences}; извлекает текст из БД \\
		\hline
	\end{tabular}
...
\begin{equation}
	...
	\qquad
	w(t, D) = 0.5 + 0.5 \cdot \frac{\text{tf}(t, D)}{\text{tf}_{\max}(D)}
	\label{eq:summary_tf}
\end{equation}

где $\text{tf}(t, S_i)$ "--- частота термина в предложении, $w(t, D)$ "--- аугментированная частота
термина в документе (коэффициент 0.5 снижает влияние сверхчастых слов),
$\text{tf}_{\max}(D)$ "--- максимальная частота термина в документе.
